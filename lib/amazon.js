import supabase, { upsert, insert, getAll, logSync } from './supabase.js';
...
// ── SYNC ORDINI ──────────────────────────────────────────────
export async function syncAmazon(giorni = 30, dateFrom = null, dateTo = null, tabella = 'ordini_amazon') {
  const log = { piattaforma: 'Amazon', inizio: new Date().toISOString(), nuovi: 0, aggiornati: 0, cancellazioni: 0, tabella };

  try {
    let createdAfter;
    let createdBefore = dateTo || null;

    if (dateFrom) {
      // Sync per anno specifico
      createdAfter = dateFrom;
      log.incrementale = false;
      log.da = createdAfter;
    } else {
      // Sync incrementale: cerca ultimo ordine nella tabella
      try {
        const { data: ultimi } = await supabase
          ...
          .limit(1);
        if (ultimi && ultimi[0] && ultimi[0].data_ordine) {
          createdAfter = ultimi[0].data_ordine;
          log.incrementale = true;
          log.da = createdAfter;
        }
      } catch(e) {}
      if (!createdAfter) {
        createdAfter = new Date(Date.now() - giorni * 24 * 60 * 60 * 1000).toISOString();
        log.incrementale = false;
      }
    }
    let nextToken = null;
    ...
    do {
      ...
      for (const ord of orders) {
        const result = await upsertOrder(ord, magazzino, tabella);
...
async function upsertOrder(ord, magazzino, tabella = 'ordini_amazon') {
